...
set obs 49
gen time = _n -25
save "$datafiles/Stata_Results/results_looking.dta", replace emptyok
...
qui xtreg looking `X`i'', fe  vce(cluster sippid)
...
use "$datafiles/Stata_Results/results_looking.dta", clear
qui {
         gen b_X`i'=.
		 		 forval j=1/49{       
							local var`j'=`j'+0
		...
		save "$datafiles/Stata_Results/results_looking.dta", replace  
}
