...
* Run unweighted regressions, store results
foreach y in rm_lfp working looking  {
	foreach i of numlist 1 2 3 4 5 {
		use  "$datafiles\Stata_Regressions\US_Paid_leave_analysis_altered.dta", clear
		local X1  " i.rhcalyr i.lBirth i.rhcalyr#i.state i.lBirth#i.state i.lBirth#i.rhcalyr _LlBiXpos_2_1 _LlBiXpos_8_1-_LlBiXpos_50_1  "
		local X2  " i.rhcalyr i.lBirth i.rhcalyr#i.state i.lBirth#i.state i.lBirth#i.rhcalyr _LlBiXpos_2_1 _LlBiXpos_8_1-_LlBiXpos_50_1 if  lt_college==0 "
		local X3  " i.rhcalyr i.lBirth i.rhcalyr#i.state i.lBirth#i.state i.lBirth#i.rhcalyr _LlBiXpos_2_1 _LlBiXpos_8_1-_LlBiXpos_50_1 if  lt_college==1 "
		local X4  " i.rhcalyr i.lBirth i.rhcalyr#i.state i.lBirth#i.state i.lBirth#i.rhcalyr _LlBiXpos_2_1 _LlBiXpos_8_1-_LlBiXpos_50_1 if  blue_collar==0 "
		local X5  " i.rhcalyr i.lBirth i.rhcalyr#i.state i.lBirth#i.state i.lBirth#i.rhcalyr _LlBiXpos_2_1 _LlBiXpos_8_1-_LlBiXpos_50_1 if  blue_collar==1 "
		qui xtreg `y' `X`i'', fe  vce(cluster sippid)
		estimates save Stata_ster/`y'_model_`i'_unweighted, replace 
	}
}
...
set obs 49
gen time = _n -25
save "$datafiles/Stata_Results/results_main_weighted.dta", replace emptyok

* Create an datafile to store unweighted coefficients 
clear
set obs 49
gen time = _n -25
save "$datafiles/Stata_Results/results_main_unweighted.dta", replace emptyok

* Use stored weighted results, create files for figures
foreach y in rm_lfp working looking  {
	foreach i of numlist 1 2 3 4 5 {
		use "$datafiles/Stata_Results/results_main_weighted.dta", clear
		...
		save "$datafiles/Stata_Results/results_main_weighted.dta", replace  
	}
}

* Use stored unweighted results, create files for figures
foreach y in rm_lfp working looking  {
	foreach i of numlist 1 2 3 4 5 {
		use "$datafiles/Stata_Results/results_main_unweighted.dta", clear
		...
		save "$datafiles/Stata_Results/results_main_unweighted.dta", replace  
	}
}
